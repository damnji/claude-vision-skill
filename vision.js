...
async function main() {
  ...
  if (!isUrl) {
    ...
    if (!fs.existsSync(resolved)) {
      console.error("识图失败:", `文件不存在: ${resolved}`);
      process.exitCode = 1;
      return;
    }
  }
  ...
  try {
    ...
    if (!bmpSwitched && CFG.dualModel && isConfigured(CFG.fallback)) {
      if (isBmp && !CFG.fallback.bmp) {
        ...
        console.error(`[vision] 主服务(${CFG.primary.provider || "未命名"}) 失败: ${err.message}`);
      ...
      console.error(`[vision] 尝试备用服务(${CFG.fallback.provider || "未命名"})...`);
      try {
        const result = await callVision(CFG.fallback, imageUrl, prompt);
        console.log(result);
        console.error(`[vision] 本次由备用服务(${CFG.fallback.provider || "未命名"}) 响应。`);
        return;
      } catch (fbErr) {
